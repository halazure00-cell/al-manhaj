...
async function startServer() {
  ...
  app.post("/api/ai/chat", async (req, res) => {
    ...
    try {
      ...
      // res.json({ debug: { GEMINI_API_KEY: process.env.GEMINI_API_KEY?.substring(0, 10), NEXT_PUBLIC_GEMINI_API_KEY: process.env.NEXT_PUBLIC_GEMINI_API_KEY?.substring(0, 10), API_KEY: process.env.API_KEY?.substring(0, 10) } });
      ...
      if (errorMessage.includes("API key not valid") || errorMessage.includes("API_KEY_INVALID")) {
        return res.json({
          response: "⚠️ **API Key Gemini tidak valid.**\n\nSistem mendeteksi bahwa API Key yang Anda masukkan salah.\n\n**Cara memperbaiki:**\n1. Buka menu **Settings** (ikon roda gigi) di AI Studio.\n2. Cari bagian **Secrets** atau **Environment Variables**.\n3. Hapus variabel `GEMINI_API_KEY` agar sistem menggunakan kunci bawaan, **ATAU** masukkan API Key Anda yang asli dan valid.\n4. Muat ulang (refresh) halaman ini.",
          context: { notes: [], books: [] }
        });
      }
